...
时效奖惩   按时 +2 / 超时 -2     # 仅限设了截止（due_at）的任务；无限期不奖不罚
...
| 榜 | 口径 | 用途 |
|---|---|---|
| 人类员工榜 | 积分降序 | 谁干活好干活多（部门/积分/完成数/驳回/**平均评分**/**按时完成率**/成本） |
| AI 员工榜 | 积分降序 | 同口径管 AI 团队 |
| ROI 统一榜 | 积分/成本 降序 | 跨族群公平对比（AI 成本低是真实优势；未配成本单价者不入榜） |

- 质量维度 = 平均验收评分（task completion 落账 `score`）；时效维度 = 按时完成率
  （分母只含设了截止的任务，无限期任务不计入——不虚设截止刷分）；
- 任务可设截止：提交时带 `due_at`（ISO 时间或日期，仅日期视为当天 23:59:59）；
  验收时用任务完成时刻 vs 截止判定按时/超时；
...
任务提交（看板表单 / `POST /api/task/submit`）可带 `due_at` 截止时间：

- 格式：ISO 时间（`2026-12-31T18:00:00`）或日期（`2026-12-31`，视为当天 23:59:59）；
  无效格式返回 400；缺省 = 不限期；
- 验收时以任务完成时刻（状态机 `updated_at`）对比截止：按时 `+2` 分、超时 `-2` 分，
  并入本次验收积分；无限期任务不奖不罚（防虚设截止刷分）；
- 账本按完成记账 `score` + `on_time`，绩效面板 / 积分榜新增
  **平均评分**、**按时完成率**（分母只含设了截止的任务）两列。

### 个人任务计划（GET /api/plans）

每人可在看板查看自己的计划任务视图，按周期分组，含**被动分配**与**个人计划**两类来源
及完成情况：

- 提交任务时可指定 **计划周期** `plan_horizon`：`day`（日）/ `week`（周）/ `month`（月）/
  `quarter`（季度）/ `half_year`（半年）/ `year`（年）；缺省 = 未分类；
- **来源自动判定**：`created_by == assignee` → 个人计划（自己提的自己做）；
  有承接人但创建者不同 → 被动分配（别人派的）；无承接人 → 未指派；
- `Task.assignee` 为**持久字段**（派单时写入）：任务完成出队后仍可按人追溯，
  不依赖工位队列；
- 每个周期组显示：总数 / 完成数 / 完成率 / 按时数 / 超时数；每组任务行含
  状态、来源、截止、时效（按时/超时/—）；
- 顶部总汇总（overall）：全范围完成率与按时/超时统计；
- 可见范围（RBAC）：admin 可查任何人（缺省全公司汇总）；manager 缺省本部门
  （含自己，可查部门成员）；staff 强制仅本人（查别人也会拦回自己）。

```
GET /api/plans                 # 按权限汇总（admin=全公司 / manager=本部门 / staff=本人）
GET /api/plans?who=emp-chen    # 查指定人（admin 任意；manager 限本部门；staff 恒为自己）
```
