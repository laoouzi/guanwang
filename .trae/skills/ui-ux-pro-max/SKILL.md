...
**Trae environment note:** this skill is installed at `.trae/skills/ui-ux-pro-max/` (absolute path `/workspace/.trae/skills/ui-ux-pro-max/`). If `${CLAUDE_PLUGIN_ROOT}` is not defined in this workspace, use the skill's own directory instead:

```bash
python "/workspace/.trae/skills/ui-ux-pro-max/scripts/search.py" "<query>" --domain <domain>
```
